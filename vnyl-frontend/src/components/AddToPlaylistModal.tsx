...
const AddToPlaylistModal: React.FC<AddToPlaylistModalProps> = ({ trackId, onClose }) => {
    ...
    const handleCreatePlaylist = async (title: string, description: string, isPublic: boolean) => {
        try {
            ...
            const data = await response.json();
            if (data.status === 'success') {
                // If it's a create-from-add flow, we should probably add the track immediately
                    ...
                    <div className="max-h-[340px] overflow-y-auto custom-scrollbar">
                        {loading ? (
                            ...
                            <div className="space-y-0.5 px-2 pb-2">
                                ...
                                {/* Add "Create New" directly to the list too for convenience if playlists exist */}
                                <button
                                    onClick={() => setIsCreateModalOpen(true)}
                                    className="w-full flex items-center gap-3.5 p-2.5 rounded-xl hover:bg-white/10 transition-colors group text-[var(--accent)] mt-2"
                                >
                                    <div className="w-11 h-11 rounded-[8px] bg-[var(--accent)]/10 flex items-center justify-center shrink-0 border border-[var(--accent)]/20 group-hover:bg-[var(--accent)]/20 transition-colors">
                                        <Plus size={18} />
                                    </div>
                                    <div className="font-semibold text-[14px]">Create New Playlist</div>
                                </button>
                            </div>
                        ) : (
                            <div className="text-center py-12 px-6">
                                ...
                                <button
                                    onClick={() => setIsCreateModalOpen(true)}
                                    className="mt-3 text-[var(--accent)] text-[13px] font-semibold hover:opacity-80 transition-opacity"
                                >
                                    Create New Playlist
                                </button>
                            </div>
